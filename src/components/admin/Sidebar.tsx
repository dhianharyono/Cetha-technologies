...
export default function Sidebar() {
  ...
  return (
    <aside className='w-64 shrink-0 h-screen bg-[#07090E] border-r border-white/5 hidden md:flex flex-col z-20 sticky top-0'>
      <div className='h-16 flex items-center px-6 border-b border-white/5'>
        <h1 className='text-xl font-black bg-linear-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent'>
          Cetha Admin
        </h1>
      </div>
      ...
      <nav className='flex-1 overflow-y-auto py-6 px-4 flex flex-col gap-2'>
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              <Icon
                className={`w-5 h-5 ${isActive ? 'text-cyan-400' : 'text-slate-500 group-hover:text-cyan-400/70'}`}
              />
