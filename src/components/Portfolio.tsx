...
import { ExternalLink, Layout, Code2, Globe } from 'lucide-react';
...
export default function Portfolio() {
  ...
  const PortfolioSkeleton = () => (
    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8'>
      {[1, 2, 3].map((i) => (
        <Card key={i} className='overflow-hidden border border-white/5 bg-[#131826]/40 backdrop-blur-sm'>
        ...
        <div className='text-center max-w-3xl mx-auto mb-16 md:mb-24'>
          ...
          >
            <h2 className='text-lg md:text-5xl font-extrabold text-white mb-6 tracking-tight'>
              Karya <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500">Kami</span>
            </h2>
...
function PortfolioCard({ project, index, isMobile = false }: { project: any, index: number, isMobile?: boolean }) {
  return (
    <motion.div
    ...
    >
      <Card className='h-full bg-[#131826]/60 backdrop-blur-xl border border-white/5 hover:bg-[#1A2235]/80 hover:border-cyan-500/30 transition-all duration-500 group overflow-hidden relative flex flex-col rounded-3xl'>
        <div className='relative h-56 w-full overflow-hidden shrink-0'>
          <div className='absolute inset-0 bg-gradient-to-t from-[#131826] to-transparent z-10 opacity-60'></div>
        ...
        <CardContent className='p-6 relative flex flex-col flex-1'>
          ...
          <div className='mt-auto pt-6 border-t border-white/5 flex items-center justify-between'>
            <div className='flex items-center gap-2 text-cyan-500/80'>
              <Layout size={16} />
              <span className='text-[10px] font-bold uppercase tracking-widest'>Paket {project.paket || 'Custom'}</span>
            </div>
          </div>
        </CardContent>
      </Card>
