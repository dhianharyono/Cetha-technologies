...
import { Zap, Smartphone, Search, Puzzle, ArrowRight } from 'lucide-react';
...
export default function Services() {
  ...
  const ServiceSkeleton = () => (
    <div className='grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8'>
      {[1, 2, 3, 4].map((i) => (
        <Card key={i} className='h-64 border border-white/5 bg-[#131826]/40 backdrop-blur-sm p-6'>
        ...
        <div className='text-center max-w-3xl mx-auto mb-16 md:mb-24'>
          ...
          >
            <h2 className='text-lg md:text-5xl font-extrabold text-white mb-6 tracking-tight'>
              Layanan <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-400">Unggulan</span>
            </h2>
            ...
            >
              {services.map((service, index) => (
                <motion.div
                  ...
                  className='group'
                >
                  <Card className={`h-full border border-white/5 bg-[#131826]/60 backdrop-blur-xl transition-all duration-500 relative overflow-hidden flex flex-col p-6 rounded-2xl ${service.borderColor} hover:shadow-2xl hover:shadow-cyan-950/20`}>
                    {/* Floating Glow */}
                    <div className={`absolute -right-8 -top-8 w-24 h-24 rounded-full blur-3xl opacity-0 group-hover:opacity-20 transition-opacity duration-500 ${service.bg}`}></div>

                    <div className={`w-12 h-12 md:w-14 md:h-14 rounded-2xl ${service.bg} flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-500 border border-white/5`}>
