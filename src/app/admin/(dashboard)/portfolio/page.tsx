...
import { getPortfolios, addPortfolio, updatePortfolio, deletePortfolio, togglePortfolioVisibility } from '@/app/actions/adminActions';
...
export default function PortfolioPage() {
    ...
    const handleOpenAdd = () => {
        setIsEditMode(false);
        setFormData({ title: '', description: '', image: '', fitur: '', paket: 'Pro Business' });
        setIsModalOpen(true);
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const payload = {
                ...formData,
                fitur: formData.fitur.split(',').map(f => f.trim()).filter(Boolean),
            };
    ...
    return (
        <div className="space-y-6 relative">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-black text-white tracking-tight">Kelola Portofolio</h1>
                    <p className="text-slate-400 mt-1">Daftar proyek klien untuk ditampilkan di landing page.</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {isLoading ? (
                    <p className="text-slate-500 col-span-3">Memuat portofolio...</p>
                ) : portfolios.length === 0 ? (
                        ...
                        <div key={item._id} className={`bg-[#131826]/80 backdrop-blur-md border border-white/5 rounded-2xl overflow-hidden group transition-all ${item.isHidden ? 'opacity-50 grayscale hover:grayscale-0' : 'hover:border-white/20'}`}>
                            <div className="relative h-48 w-full bg-[#0B101C]">
                                ...
                                <div className="absolute top-3 right-3 flex gap-2">
                                    ...
                                    >
                                        {item.isHidden ? <EyeOff className="w-4 h-4 text-red-400" /> : <Eye className="w-4 h-4" />}
                                    </button>
                                </div>
                            </div>
                            <div className="p-5">
                                ...
                                <div className="flex gap-2 mt-4 pt-4 border-t border-white/10">
                                    ...
                                    <button onClick={() => handleDeleteParams(item._id)} className="flex-1 flex items-center justify-center py-2 bg-white/5 hover:bg-red-500/20 text-slate-300 hover:text-red-400 rounded-lg text-sm transition-colors gap-2">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
                    ...
                    <div className="relative w-full max-w-lg bg-[#131826] border border-white/10 rounded-2xl p-6 shadow-2xl z-10">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-slate-300 mb-1">Judul Proyek</label>
                                <input required type="text" value={formData.title} onChange={e => setFormData({ ...formData, title: e.target.value })} className="w-full px-3 py-2 bg-[#0B101C]/50 border border-white/10 rounded-lg text-white outline-none focus:border-cyan-500 transition-colors text-sm" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-300 mb-1">Deskripsi & Proses</label>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-medium text-slate-300 mb-1">Paket Pemesanan</label>
                                    <input required type="text" value={formData.paket} onChange={e => setFormData({ ...formData, paket: e.target.value })} className="w-full px-3 py-2 bg-[#0B101C]/50 border border-white/10 rounded-lg text-white outline-none focus:border-cyan-500 transition-colors text-sm" placeholder="Misal: Enterprise" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-slate-300 mb-1">Fitur Tersedia</label>
                                    <input required type="text" value={formData.fitur} onChange={e => setFormData({ ...formData, fitur: e.target.value })} className="w-full px-3 py-2 bg-[#0B101C]/50 border border-white/10 rounded-lg text-white outline-none focus:border-cyan-500 transition-colors text-sm" placeholder="Pisahkan dengan koma" />
                                </div>
                            </div>
                            <div className="flex gap-3 justify-end mt-6 pt-4 border-t border-white/5">
                                <button type="button" onClick={() => setIsModalOpen(false)} className="px-4 py-2 bg-white/5 hover:bg-white/10 text-white rounded-lg text-sm font-medium transition-colors">Batal</button>
                                <button type="submit" className="px-4 py-2 bg-cyan-500 hover:bg-cyan-600 text-slate-900 rounded-lg text-sm font-bold shadow-[0_0_15px_rgba(6,182,212,0.3)] transition-colors">Simpan</button>
                            </div>
                        </form>
                    </div>
                </div>
